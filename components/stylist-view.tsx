...
import { ArrowUpRight, CloudSun, LoaderCircle, RefreshCw, Shirt, Sparkles, SunMedium } from '@/components/icons';
...
export function StylistView() {
  ...
  if (!items.length) return <div className="animate-float-in">
    <div className="mb-8 flex flex-col justify-between gap-5 md:flex-row md:items-end"><div><p className="eyebrow mb-3">A személyes sztylistod</p><h1 className="font-display text-[42px] leading-none tracking-[-0.04em]">Mit vegyél fel ma?</h1><p className="mt-3 max-w-xl text-sm leading-6 text-[#85827b]">Az AI a saját gardróbodból állít össze szetteket — először töltsünk fel néhány darabot.</p></div><div className="flex items-center gap-2 text-[11px] text-[#817e76]"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#f2e5e1] text-[#aa716a]"><Sparkles size={14} /></span> Powered by WardrobeAI</div></div>
    <section className="relative flex min-h-[390px] items-center overflow-hidden rounded-2xl bg-[#252523] p-7 text-white sm:p-12"><div className="absolute -right-16 -top-20 h-72 w-72 rounded-full border border-white/10" /><div className="absolute -bottom-28 right-32 h-64 w-64 rounded-full border border-[#d7a19a]/20" /><div className="relative max-w-lg"><span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.15em] text-[#d7a19a]"><Shirt size={12} /> Saját gardrób szükséges</span><h2 className="mt-6 font-display text-[43px] leading-[0.95] tracking-[-0.04em]">Előbb töltsünk fel, aztán öltöztessünk.</h2><p className="mt-4 max-w-md text-sm leading-6 text-[#aaa8a1]">Adj hozzá legalább egy felsőt és egy alsót, hogy az AI sztylist összeállíthassa az első személyes outfitjeidet.</p><Link href="/wardrobe/new" className="mt-7 inline-flex h-11 items-center gap-2 rounded-full bg-[#d7a19a] px-5 text-xs font-bold text-[#302524] transition hover:bg-[#e5b7b0]">Ruhák hozzáadása <ArrowUpRight size={14} /></Link></div></section>
  </div>;
